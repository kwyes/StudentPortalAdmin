...
var studentLeaveBanResponse;
function getStudentLeaveBan(){
  var eData = [];
  var table = $("#datatables-leave-ban").DataTable();


  $.ajax({
    url: "../ajax_php/a.getStudentLeaveBan.php",
    type: "POST",
    dataType: "json",
    success: function (response) {
      studentLeaveBanResponse = response;
      console.log(response);
      var tr;
      if (response.result == 0) {
        tr = '<tr><td colspan="7">No Record</td></tr>';
      } else {
        for (let i = 0; i < response.length; i++) {
          var title =
            '<a href="" data-id="' +
            response[i].BanID +
            '" data-toggle="modal" class="LeaveBanDetailMdlLink">' +
            response[i].BanID +
            "</a>";

          var name;
          if (response[i].EnglishName) {
            name = response[i].FirstName + " (" + response[i].EnglishName + ") " + response[i].LastName;
          } else {
        ...
        table = $("#datatables-leave-ban").DataTable({
          data: eData,
          deferRender: true,
          bDestroy: true,
          autoWidth: false,
